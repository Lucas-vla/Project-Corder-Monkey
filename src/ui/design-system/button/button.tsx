...
export const Button = ({
  // Par exemple si je defini mon composant sans taille sans variant... ceux-ci seront les valeurs par defaut.
  size = "medium",
  variant = "accent",
  icon,
  iconTheme = "accent",
  ...
  switch (variant) {
    ...
    case "secondary":
      variantStyles =
        "bg-primary-200 hover:bg-primary-300/50 text-primary rounded"; // la deuxième valeur de bg-primary-300/50 est pour l'oppacité
      break;
    case "outline":
      variantStyles =
        "bg-white hover:bg-gray-400/50 border-gray-500 text-gray-900 rounded";
      break;
    case "disabled":
      variantStyles =
  ...
  const buttonElement = (
    <button
      ...
      onClick={handleClick}
      disabled={disabled || isLoading ? true : false} // ici on indique que le bouton sera désactivé en cas de loading ainsi que si on lui spécifie d'etre désactivé (très imporant de le désactivé lors d'un chargement pour ne pas que l'utilisateur solicite plusieurs fois l'API).
    >
      {buttonContent}
    </button>
  );
